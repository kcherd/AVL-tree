...
public:
	BST(); //конструктор
	BST(const BST<T1, T2> &tree); // конструктор копировани€
	~BST(); // деструктор
	...
	class Iterator {
		...
		node<T1, T2>* cur; //”казатель на текущий элемент дерева
		void prev()
		{
	...
	public:
		Iterator(); //¬ ќ“„≈“ Ќ≈ ¬—“ј¬Ћя“№
		Iterator(BST<T1, T2>* t, node<T1, T2>* n); //¬ ќ“„≈“ Ќ≈ ¬—“ј¬Ћя“№
...
bool BST <T1, T2>::erase(T1 k)
{
	...
	node<T1, T2> *savedSubtree; //то, что идет после удал€емого элемента
	node<T1, T2> *minRight;

	while (temp != NULL && temp->key != k) // поиск удал€емого элемента
	{
		parent1 = temp;
		numOperation++;
	...
	if (parent1 == NULL) //если удал€емый элемент лежит в корне
		root = savedSubtree;
...
int BST <T1, T2>::keyNumber(T1 k)
{
	...
	while (temp != NULL)
	{
		...
		{
			if (temp->right != NULL)
				num += temp->right->num_descendants + 2; //потомки правого поддерева, оно само и текуща€ вершина
			else num++;
			temp = temp->left;
		}
		else
